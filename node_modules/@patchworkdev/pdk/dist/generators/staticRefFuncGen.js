...
exports.StaticRefFuncGen = void 0;
const generator_1 = require("../generator");
class StaticRefFuncGen {
    gen(schema) {
        ...
        function generateAddReferenceFunction(entries) {
                ...
                if (entry.fieldType == "literef") {
                    ...
                    for (let i = 0; i < entry.arrayLength; i++) {
                        ...
                        addReferenceLines.push(`if (uint64(slot${shift}) == 0) {\n` +
                            (0, generator_1.ind)(indent * 4, `        mdStorage[${slot}] = slot | uint256(liteRef)${shift !== "" ? ` << ${offset}` : ""};\n` +
                                `    }`));
        ...
        ;
        function generateAddReferenceBatchFunction(entries) {
            if (schema.hasLiteRef()) {
                let out = `` +
        ...
        const addReferenceBatchDirectFunction = `function addReferenceBatch(uint256 tokenId, uint64[] calldata liteRefs, uint256 targetMetadataId) external {\n` +
            `    if (targetMetadataId != 0) {\n` +
            `        revert UnsupportedMetadataId();\n` +
            `    }\n` +
        ...
        function startSlot(which) {
            return schema.liteRefSlotNumber(which);
        }
        function endSlot(which) {
...
exports.StaticRefFuncGen = StaticRefFuncGen;
