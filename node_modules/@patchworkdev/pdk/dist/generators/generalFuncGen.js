...
class GeneralFuncGen {
    ...
    getSupportsInterfaceFunc(schema) {
        ...
        if (features.some((feature) => feature === contractSchema_1.Feature.PATCH)) {
            if (features.some((feature) => feature === contractSchema_1.Feature.REVERSIBLE)) {
        ...
        if (features.some((feature) => feature === contractSchema_1.Feature.MINTABLE)) {
        ...
        func += `    return `;
        support.map((value) => { func += `type(${value}).interfaceId == interfaceID ||\n        `; });
        if (bases.length === 1) {
...
exports.GeneralFuncGen = GeneralFuncGen;
